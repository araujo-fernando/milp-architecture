...
class SolutionReporter:
    ...
    def __init__(self, data: InstanceData, builder: CVRPBuilderCplex | CVRPBuilderDocplex, solution: Any, build_seconds: float, solve_seconds: float):
        self.data = data
    ...
    def extract(self) -> dict[str, Any]:
        ...
        if isinstance(self.builder, CVRPBuilderCplex):
            ...
            variable_count = self.builder.model.number_of_variables
            constraint_count = self.builder.model.number_of_constraints
        ...
        return {"execucao": {"id_execucao": f"RUN-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{self.data.depot}", "instancia": f"{self.data.depot}/{self.data.delivery_date}", "gerado_em": datetime.now().astimezone().isoformat(), "versao_modelo": "cvrp-3idx-1.0.0", "hash_instancia": f"sha256:{sha256(repr(self.data).encode()).hexdigest()[:16]}", "moeda": self.data.currency}, "status_solver": {"status": status, "objetivo": objective, "tempo_build_s": round(self.build_seconds, 4), "tempo_solve_s": round(self.solve_seconds, 4), "num_variaveis": variable_count, "num_restricoes": constraint_count}, "instancia_resumo": {"clientes_elegiveis": len(self.data.demand_kg), "clientes_atendidos": len(self.data.demand_kg), "clientes_descartados": list(self.data.discarded), "veiculos_disponiveis": len(self.data.vehicles), "veiculos_utilizados": len(routes), "demanda_total_kg": total_load, "capacidade_total_kg": sum(vehicle.capacity_kg for vehicle in self.data.vehicles)}, "kpis_globais": {"custo_total": objective, "distancia_total_km": total_distance, "carga_total_kg": total_load, "paradas_total": len(self.data.demand_kg)}, "rotas": routes, "diagnostico": []}
